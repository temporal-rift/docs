...
State is persisted in the same transaction as the aggregate mutation or event that triggered the step. If a failure must be recorded despite rollback, the compensation path persists that failure state in a separate transaction.

## Sagas summary

| Saga | Service | Trigger | Key complexity |
|---|---|---|---|
| `StartGameSaga` | game-service / session | `StartGame` command | Faction assignment atomicity |
...
| `EndGameSaga` | game-service / session | Win / collapse / stabilize | Resource cleanup |
| `WeaverChainSaga` *(planned — [details](/sagas/special#weaverchainsaga))* | timeline-service | `THREAD` special action | Only saga spanning multiple eras |
...
```mermaid
graph TD
  GS[StartGameSaga] --> E1[EraSaga era 1]
  ...
  PR -->|3rd cascade| GE[EndGameSaga]
  E1 -->|no winner| E2[EraSaga era N+1]
  E2 -->|no winner| EN[...]

  WCS[WeaverChainSaga — planned, spans era boundaries]
