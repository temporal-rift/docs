...
Three deployables: `game-service`, `timeline-service`, and `read-service`.
...
graph TD
  ...
  TL -->|OutcomeApplied, EraResolutionCompleted| SC
  TL -->|EraResolutionCompleted| S
  TL --> P & N
  S --> P & N
  A --> P & N
  SC --> P & N
...
Every implemented service emits Micrometer traces to Zipkin. A trace can follow an HTTP request through direct Kafka
listeners and producers. In `game-service` and `read-service`, it also covers cross-module Spring Modulith event
listeners; in `timeline-service`, it covers the transactional-outbox Spring event listener and Kafka relay.
Propagation headers are transport metadata; the documented event headers and payload schemas do not change.
