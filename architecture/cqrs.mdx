...
<Note>
Different fields of the same read model can catch up at slightly different times — for example, a round
transition can appear before a just-played card has been dropped from the hand shown in the same response. Treat
a rejection like `422-01` (Card not in hand) right after observing a state change as a cue to refresh and retry,
not as a hard error to show the player.
</Note>
