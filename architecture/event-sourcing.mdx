...
`timeline-service` is fully event-sourced. Its `FutureEvent` aggregate is rebuilt by replaying its stored event history rather than reading a current-state row.
